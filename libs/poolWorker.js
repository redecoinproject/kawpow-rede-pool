	...
	Object.keys(poolConfigs).forEach(function(coin) {
		...
		var authorizeFN = function(ip, port, workerName, password, extraNonce1, version, callback) {
			handlers.auth(port, workerName, password, function(authorized) {
				...
				logger.debug('AUTH> authstr [%s] worker [%s] passwd [%s] ip [%s]', authString, workerName, password, functions.anonymizeIP(ip));
				callback({
					error: null,
		...
		pool.on('share', function(isValidShare, isValidBlock, data) {
			...
			if (workerInfo.length === 2) {
				if (isValidShare) {
					if (data.shareDiff > 1000000000) {
						logger.warn('\033[41mSHARE>WARN invalid share found!!\033[0m');
					} else if (data.shareDiff > 1000000) {
						logger.warn('\033[41mSHARE>WARN invalid share found!!\033[0m');
                                        } else if (data.shareDiff == 0.00000000) {
                                                logger.warn('\033[41mSHARE>WARN invalid share found!!\033[0m', data.job, data.difficulty, data.shareDiff, workerInfo[0], workerInfo[1], functions.anonymizeIP(data.ip));
					}
					logger.info('SHARE job: %s req: %s res: %s by %s worker: %s [%s]', data.job, data.difficulty, data.shareDiff, workerInfo[0], workerInfo[1], functions.anonymizeIP(data.ip));
				}
				else if (!isValidShare) {
					logger.info('\033[41mREJECTED job: %s diff: %s by %s worker: %s reason: %s [%s]\033[0m', data.job, data.difficulty, workerInfo[0], workerInfo[1], data.error, functions.anonymizeIP(data.ip));
				}
			} else {
				if (isValidShare) {
					if (data.shareDiff > 1000000000) {
					...
					logger.info('ACCEPTED job: %s req: %s res: %s by %s worker: none [%s]', data.job, data.difficulty, data.shareDiff, workerStr, functions.anonymizeIP(data.ip));
				} else if (!isValidShare) {
					logger.info('\033[41mREJECTED job: %s diff: %s by %s worker: none reason: %s [%s]\033[0m', data.job, data.difficulty, workerStr, data.error, functions.anonymizeIP(data.ip));
				}
			}
			...
			let workerInfo = workerStr.split('.');
			if (workerInfo.length === 2) {
				logger.info('DIFFUPDATE diff: %s miner: %s worker: %s', diff, workerInfo[0], workerInfo[1]);
			} else {
				logger.info('DIFFUPDATE diff: %s miner: %s worker: none', diff, workerStr);
			}
